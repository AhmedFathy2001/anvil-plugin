...
{
	private static TrackedDrop drop(int id, String label, int cur, int req, Integer... items)
	{
	...
	private static TrackedStat stat(int id, String label, int cur, int goal)
	{
	...
	public void multiTagCategoriesSplitAndFilter()
	{
		...
		rows.add(new TaskRow(1, "Zuk cape", Type.DROP, 0, 1, -1, 5, null, "Inferno, PvM"));
		rows.add(new TaskRow(2, "99 Slayer", Type.STAT, 0, 1, -1, 5, null, "Skilling"));
	...
	{
		assertEquals(Status.NOT_STARTED, ClogTaskModel.statusOf(0, 5));
		assertEquals(Status.IN_PROGRESS, ClogTaskModel.statusOf(2, 5));
		assertEquals(Status.COMPLETED, ClogTaskModel.statusOf(5, 5));
		assertEquals(Status.COMPLETED, ClogTaskModel.statusOf(7, 5));
		// Untargeted tile (goal <= 0): any progress completes it.
		assertEquals(Status.NOT_STARTED, ClogTaskModel.statusOf(0, 0));
		assertEquals(Status.COMPLETED, ClogTaskModel.statusOf(1, 0));
	}
	...
	public void buildMergesDropsAndStatsWithIcons()
	{
		...
		TaskRow dropRow = rows.get(0);
		assertEquals(Type.DROP, dropRow.type);
		assertEquals(12921, dropRow.itemId); // first tracked item id
		assertEquals(Status.IN_PROGRESS, dropRow.status);

		TaskRow statRow = rows.get(1);
		assertEquals(Type.STAT, statRow.type);
	...
	{
		List<TaskRow> rows = new ArrayList<>();
		rows.add(new TaskRow(1, "Done drop", Type.DROP, 5, 5, 100));
	...
	public void buildClassifiesEveryKind()
	{
		...
		TrackedDrop coll = drop(2, "Void set", 0, 1, 11665);
		...
		cfg.trackedStats = new ArrayList<>();
		TrackedStat skill = stat(3, "Agility XP", 0, 100);
		skill.statType = "skill";
		cfg.trackedStats.add(skill);
		TrackedStat boss = stat(4, "Zulrah KC", 0, 100);
		boss.statType = "boss";
		cfg.trackedStats.add(boss);

		cfg.trackedKills = new ArrayList<>();
		TrackedKill kill = new TrackedKill();
		kill.tileId = 5;
		...
		TrackedTimed timed = new TrackedTimed();
		timed.tileId = 6;
		...
		assertEquals(Kind.DROP, kindOf(rows, 1));
		assertEquals(Kind.COLLECTION, kindOf(rows, 2));
		assertEquals(Kind.SKILL, kindOf(rows, 3));
		assertEquals(Kind.BOSS, kindOf(rows, 4));
		assertEquals(Kind.KILL, kindOf(rows, 5));
		assertEquals(Kind.TIMED, kindOf(rows, 6));
	...
	private static TierBand band(String key, String label, int min)
	{
	...
	private static TaskRow rowOf(List<TaskRow> rows, int tileId)
	{
		for (TaskRow r : rows)
		{
			if (r.tileId == tileId)
	...
	private static Kind kindOf(List<TaskRow> rows, int tileId)
	{
	...
	{
		List<TaskRow> rows = new ArrayList<>();
		rows.add(new TaskRow(1, "Zeta done", Type.DROP, 1, 1, 100));
	...
	private static ItemRequirement req(String name, int have, String group, int groupRequire)
	{
	...
	private static TrackedValue value(String mode, long threshold, long current)
	{
		TrackedValue v = new TrackedValue();
		v.tileId = 1;
		v.label = "Bank it";
		v.mode = mode;
